...
export default function AdminOperations() {
  ...
  const [hideDummyUsers, setHideDummyUsers] = useState(() => {
    try {
      return localStorage.getItem("adminOperations.hideDummyUsers") === "true";
    } catch (err) {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("adminOperations.hideDummyUsers", hideDummyUsers ? "true" : "false");
    } catch (err) {
  ...
  const autoGeneratedBuyerIds = new Set(
    requirements
      .filter((requirement) => requirement?.isAutoGenerated)
      .map((requirement) => String(requirement?.buyerId?._id || requirement?.buyerId || ""))
      .filter(Boolean)
  );
  ...
  const isDummyUser = (user) => {
    ...
    const hasContactOrName = Boolean(
      String(user?.email || "").trim() ||
      String(user?.mobile || user?.phone || "").trim() ||
      String(user?.name || "").trim()
    );
    const isBuyerOnly = Boolean(user?.roles?.buyer) && !user?.roles?.seller && !user?.roles?.admin;
    return (
      autoGeneratedBuyerIds.has(String(user?._id || "")) ||
      contact === "+919999999999" ||
      contact === "9999999999" ||
      buyerName === "demo buyer" ||
      (isBuyerOnly && !hasContactOrName)
    );
  };
